...
lateinit var addNewNote: FloatingActionButton
...
class Home : Fragment() {
    ...
    override fun onCreateView(
        ...
        addNewNote = view.findViewById(R.id.btnAddNewNote)

        CoroutineScope(Dispatchers.IO).launch {
            allNotes = noteDatabase.getNoteDao().getAllNotes() as ArrayList<Note>
            withContext(Dispatchers.Main){
                adapter = RVNotes(allNotes,requireContext())
        ...
        addNewNote.setOnClickListener {
            Navigation.findNavController(view).navigate(R.id.action_home2_to_addNewNote)
        }
